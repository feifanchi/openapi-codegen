...
export class Vue3AxiosService {
    generateService(domainPath: string, tag: Tag): string {
        ...
        for (let method of tag.methods.sort((a, b) => a.operationId.localeCompare(b.operationId))) {
            ...
            rets.push(`return service.${method.requestMethod}(` + '`' + path + '`' + ``);
            ...
            if (method.response) {
                ...
                if (type.arrayLevel) {
                    ...
                    if (type.type === TypeEnum.BASIC_NEW || type.type === TypeEnum.OUTER_NEW) {
                        rets.push(`.then((res:${type.name})=> new ${type.name}(res));`);
                    } else if (type.type === TypeEnum.INNER) {
                        rets.push(`.then((res:${type.name})=> new C${type.name}(res));`);
                    } else if (type.type === TypeEnum.ENUM) {
                        rets.push(`.then((res:${type.name})=> res as ${type.name});`);
                    }
                }
            }
            rets.push(`}\n`)
        ...
        rets.push(`}`);
        return ['import {' + [...importInners.keys()].join(',') + `} from '${domainPath}';`, ...imports.keys(), ...rets].join(' ');
    }
}
